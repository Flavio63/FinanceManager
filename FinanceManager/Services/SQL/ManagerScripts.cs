...
namespace FinanceManager.Services.SQL
{
    public class ManagerScripts
    {
        ...
        /// </summary>
        public static readonly string GetManagerLiquidAssetListByOwner = "SELECT id_liquid_movement, id_portafoglio, desc_portafoglio, id_location, desc_location, AA.id_valuta, " +
            "cod_valuta, id_tipoMovimento, desc_Movimento, id_titolo, BB.Isin, AA.id_borsa, CC.desc_borsa, data_movimento, ammontare, shares_quantity, unity_local_value, total_commission_euro, " +
            "disaggio_cedole, valore_cambio, disp, note FROM ( " +
            ...
            "data_movimento, shares_quantity, unity_local_value, total_commission_euro, disaggio_cedole, ammontare, valore_cambio, if(disponibile = 1, 'true', 'false') AS disp, note " +
            ...
            ") AA LEFT JOIN daf_titoli BB ON BB.id_tit = AA.id_titolo left join daf_borsa CC ON CC.id_borsa = AA.id_borsa ORDER BY data_movimento DESC";
        ...
        /// </summary>
        public static readonly string GetManagerSharesMovementByOwnerAndLocation = "SELECT id_liquid_movement, id_portafoglio, desc_portafoglio, id_location, desc_location, AA.id_valuta, " +
            "cod_valuta, id_tipoMovimento, desc_Movimento, id_titolo, BB.Isin, AA.id_borsa, CC.desc_borsa, data_movimento, ammontare, shares_quantity, unity_local_value, total_commission_euro, " +
            "disaggio_cedole, valore_cambio, disp, note FROM ( " +
            "SELECT id_liquid_movement, id_portafoglio, desc_portafoglio, C.id_location, desc_location, D.id_valuta, cod_valuta, E.id_tipoMovimento, desc_Movimento, id_titolo, id_borsa, " +
            "data_movimento, shares_quantity, unity_local_value, total_commission_euro, disaggio_cedole, ammontare, valore_cambio, if(disponibile = 1, 'true', 'false') AS disp, note " +
            "FROM daf_portafoglio A, daf_portfolio_owner B, daf_location C, daf_valuta D, daf_tipo_movimento E " +
            "WHERE A.id_gestione = B.id_portafoglio AND A.id_location = C.id_location AND A.id_valuta = D.id_valuta AND A.id_movimento = E.id_tipoMovimento AND B.id_portafoglio = @owner " +
            "AND C.id_location = @id_location AND id_titolo IS NOT NULL ) AA LEFT JOIN daf_titoli BB ON BB.id_tit = AA.id_titolo left join daf_borsa CC ON CC.id_borsa = AA.id_borsa ORDER BY data_movimento DESC";
        ...
        public static readonly string AddManagerLiquidAsset = "INSERT INTO daf_portafoglio (id_liquid_movement, id_gestione, id_location, id_valuta, id_movimento, " +
            "id_titolo, id_borsa, data_movimento, ammontare, shares_quantity, unity_local_value, total_commission_euro, disaggio_cedole, valore_cambio, disponibile, note) " +
            ...
            "@unity_local_value, @total_commission_euro, @disaggio_cedole, @valore_cambio, @disponibile, @note);";
        ...
        public static readonly string UpdateManagerLiquidAsset = "UPDATE daf_portafoglio SET id_gestione = @id_portafoglio, id_location = @id_location, id_valuta = @id_valuta, " +
            ...
            "unity_local_value = @unity_local_value, total_commission_euro = @total_commission_euro, disaggio_cedole = @disaggio_cedole, valore_cambio = @valore_cambio, " +
